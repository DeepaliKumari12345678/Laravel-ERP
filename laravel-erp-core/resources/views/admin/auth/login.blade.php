...
<head>
    ...
    <style>
        ...
        .box {
            width: min(420px, 92vw); background: #fff; border-radius: 16px; padding: 2rem;
        ...
        h1 { font-family: "IBM Plex Serif", serif; margin: 0 0 0.35rem; font-size: 1.7rem; text-align:center; }
        .auth-brand h1 { margin:0; }
        p { color: #667085; margin: 0 0 1.4rem; text-align:center; }
        label { display: grid; gap: 0.35rem; margin-bottom: 0.9rem; font-size: 0.88rem; color: #667085; }
        input { border: 1px solid #e4e7ec; border-radius: 8px; padding: 0.65rem 0.75rem; font: inherit; color: #102a43; }
        ...
        button {
            ...
            font-weight: 600; cursor: pointer; margin-top: 0.4rem;
        }
        ...
        .ok { background: #dcfae6; color: #027a48; padding: 0.65rem 0.8rem; border-radius: 8px; margin-bottom: 1rem; font-size: 0.9rem; }
...
<div class="box">
    ...
    <p>Sign in to the back office</p>
    ...
    <form method="post" action="{{ route('admin.login.store') }}">
        @csrf
        @php $prefillEmail = old('email', $rememberedEmail ?: 'prestaworld12@gmail.com'); @endphp
        <label>Email
            <input type="email" name="email" value="{{ $prefillEmail }}" required autocomplete="username">
        </label>
        <label>Password
            <input type="password" name="password" value="gmail" required autofocus autocomplete="current-password">
        </label>
        <div style="margin:-0.35rem 0 0.9rem;text-align:right;">
            <a href="{{ route('admin.password.request') }}" style="color:#0f6e56;font-size:0.88rem;font-weight:600;text-decoration:none;">Forgot password?</a>
        </div>
        <label style="display:flex;align-items:center;gap:0.45rem;color:#344054;">
            <input type="checkbox" name="remember" value="1" @checked(($rememberedEmail ?? '') !== '')> Remember me
        </label>
    ...
    @if($canSignup ?? false)
        <div class="foot">
            First time here? <a href="{{ route('admin.signup') }}">Create Super Admin</a>
        </div>
    @else
        <div class="foot">
            Need Super Admin access? Ask an existing Super Admin to add you under <strong>Employees</strong>.
        </div>
    @endif
</div>
</body>
