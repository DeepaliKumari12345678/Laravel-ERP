...
@section('title', 'Attribute '.$group->name)
...
<style>
    ...
    .color-swatch {
        ...
        border:1px solid var(--ps-line); vertical-align:middle; margin-right:0.4rem;
    }
...
<div style="display:flex;justify-content:space-between;align-items:center;gap:1rem;flex-wrap:wrap;margin-bottom:1rem;">
    <h1 class="page-title" style="margin:0;">Attribute {{ $group->name }}</h1>
    <div class="actions" style="display:flex;gap:0.5rem;flex-wrap:wrap;">
        <a href="{{ route('admin.catalog.attributes.values.create', $group) }}" class="btn btn-primary">+ Add new value</a>
    </div>
</div>
...
<div class="card">
    <div class="card-head"><h3 style="margin:0;">{{ $group->name }} ({{ $values->total() }})</h3></div>

    <form method="get" action="{{ route('admin.catalog.attributes.show', $group) }}" class="list-filters" data-auto-search="off">
        ...
        <div class="filter-actions">
            ...
            @if($filtersActive)
                <a href="{{ route('admin.catalog.attributes.show', $group) }}" class="btn btn-ghost">Reset</a>
            @endif
        </div>
    </form>
        ...
        <table>
            ...
            @forelse($values as $value)
                <tr>
                    <td>{{ $value->id }}</td>
                    <td>
                        @if($group->type === 'color' && $value->color)
                            <span class="color-swatch" style="background:{{ $value->color }}"></span>
                        @endif
                        {{ $value->name }}
                    </td>
                    ...
                    <td>
                        <div class="cu-actions">
                            <a class="cu-icon-btn" href="{{ route('admin.catalog.attributes.values.edit', [$group, $value]) }}" title="Edit">
                            ...
                            <div class="cu-menu">
                                ...
                                <div class="cu-menu-panel">
                                    <form method="post" action="{{ route('admin.catalog.attributes.values.destroy', [$group, $value]) }}" onsubmit="return confirm('Delete this value?')">
                                        @csrf
                                        @method('DELETE')
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;gap:1rem;flex-wrap:wrap;margin-top:1rem;">
        <a href="{{ route('admin.catalog.attributes') }}" class="btn btn-ghost">← Back to list</a>
